...
def main(argv):
  ...
  if load_layer <= student_layers[-2]:
    ...
    for i in range(student_layers.index(load_layer),n_hints):
      ...
      load_layer = student_layers[-1]
      
  if load_layer == student_layers[-1]:
    ...
    load_layer = len(student.model.layers)-1
    
  if load_layer == len(student.model.layers)-1:
    # Load pretrained student network
    fo = open(student.extensions[0].save_path[0:-4] + "_hintlayer" + str(load_layer) + "_best.pkl", 'r')
    pretrained_model = pkl.load(fo)
    fo.close()
    student.model.layers = pretrained_model.layers 
