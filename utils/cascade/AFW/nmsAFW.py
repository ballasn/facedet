...
def dummy_nms(maps, prob):
    ...
    maps_ = {}
    for s in maps:
        maps_[s] = np.copy(maps[s])
        # Apply the filter to the overlapping region
        maps_[s] = maps[s] * (maps[s] > prob)
        n_z = np.transpose(np.nonzero(maps_[s]))
        rval.extend([[s, n_z[e, 0], n_z[e, 1], maps_[s][n_z[e, 0], n_z[e, 1]]]
                     for e in range(len(n_z))])
    ...
    '''
        ...
        print "Warning : Locally lowered min prob from", prob,
        prob -= 0.1
        print 'to', prob
        return dummy_nms(maps, prob)
    '''
    return rval
